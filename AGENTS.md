...
| `content/units/*.json` | The pool. You write these. One file per source. |
...
3. Sync. User hits ROLL (max 5 per session). They can see the current roll plus 3 previous and TAKE one.
4. User studies however they want and makes a video so they actually did it.
5. They mark completed with that proof. That title never rolls again.
